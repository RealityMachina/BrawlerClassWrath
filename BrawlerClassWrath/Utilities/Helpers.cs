...
namespace BrawlerClassWrath.Utilities {
    public static class Helpers {
        ...
        public static UnityEngine.Sprite GetIcon(string assetId)
        {
            var asset = Resources.GetBlueprint<BlueprintUnitFact>(assetId);
            return asset?.Icon;
        }
        ...
        public static UIGroup CreateUIGroup(IEnumerable<BlueprintFeatureBase> features)
        {
            var result = new UIGroup(); 
             var refArray = new BlueprintFeatureBaseReference[features.Count()];

            for(var i = 0; i < refArray.Count(); i++)
            {
                refArray[i] = features.ElementAt(i).ToReference<BlueprintFeatureBaseReference>();
            }

            result.m_Features.AddRange(refArray);
            return result;
        }
        ...
        //public static FastSetter CreateFieldSetter<T, S>(string name) => CreateFieldSetter<T,S>(name);
        // public static FastGetter CreateFieldGetter<T, S>(string name) => CreateFieldGetter(typeof(T), name);
        //public delegate void FastSetter(object source, object value);
        //public delegate object FastGetter(object source);

        public static FastRef<T, S> CreateFastRef<T, S>(string name)
        {
