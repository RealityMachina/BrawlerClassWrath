    ...
    static class ModSettings {
       ...
       // public static Scaling Scaling;
        ...
        private static void LoadSettings<T>(string fileName, ref T setting) where T : IUpdatableSettings {
            ...
            var resourcePath = $"BrawlerClassWrath.Config.{fileName}";
            ...
            if (File.Exists(userPath)) {
                using (StreamReader reader = File.OpenText(userPath)) {
                    try {
                        ...
                        BrawlerClassWrath.Main.Error("Failed to load user settings. Settings will be rebuilt.");
                        try { File.Copy(userPath, userConfigFolder + $"{Path.DirectorySeparatorChar}BROKEN_{fileName}", true); } catch { BrawlerClassWrath.Main.Error("Failed to archive broken settings."); }
                    }
                }
            }
